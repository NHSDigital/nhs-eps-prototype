module.exports = (router) => {
  ...
  router.get('/epsv11/search', function (req, res) {
    // check for clear
    let refine = req.query.refine || 'false';
    let lastSearch = req.session.data["last-search"];
    let target = 'epsv11/search';
    if (lastSearch === 'basic') {
      target = 'epsv11/search-basic';
    } else if (lastSearch === 'advanced') {
      target = 'epsv11/search-advanced';
    } else {
      target = 'epsv11/search';
    }
...
router.post("/epsv11/search-presc-post", function (req, res) {
...
router.post("/epsv11/search-nhs-post", function (req, res) {
...
// change record via a NHS number on NoK section
  router.post("/epsv11/change-record", function (req, res) {
  ...
  router.post("/epsv11/switch-layout", function (req, res) {
  ...
  router.get('/epsv11/patient-details', function (req, res) {
    ...
    return res.render('epsv11/patient-details', {'target': target})
  })

  // switcher routing

  router.post('/epsv11/switch', function (req, res) {
    // grab the bit of the url we need to se the redirect to the same page
    var url = req.get('Referrer') .split( '/' );
    res.redirect('/epsv11/' + url[4] )
  })
 
  
 // Search results page
router.get("/epsv11/search-results", function (req, res, next) {
  ...
  res.render('./epsv11/search-results', {
  ...
  router.get("/epsv11/prescription-results", function (req, res, next) {
    ...
    res.render('./epsv11/prescription-results', {
 ...
 router.get("/epsv11/prescription-template", function (req, res) {
  ...
  if (!prescID || !prescType) {
    ...
    return res.redirect("/epsv11/search");
  }
  ...
  res.render("./epsv11/prescription-template", {
    prescriptionID: prescription.prescriptionID,
    prescriptionType: prescription.prescriptionType,
    prescriptionERDsupply: prescription.daysSupply, 
    pendingCancellation: prescription.pendingCancellation,
