module.exports = (router) => {
  ...
  router.get('/epsv14/search', function (req, res) {
    let refine = req.query.refine || 'false';
    let lastSearch = req.session.data["last-search"];
    let target = 'epsv14/search';


    if (lastSearch === 'basic') {
      target = 'epsv14/search-basic';
    } else if (lastSearch === 'advanced') {
      target = 'epsv14/search-advanced';
    }
    ...
    return res.render('epsv14/search', {
      'errors': errors
    });
});
...
router.post("/epsv14/search-presc-post", function (req, res) {
...
router.post("/epsv14/search-nhs-post", function (req, res) {
...
router.get('/epsv14/search-nhs', function (req, res) {
  let errorType = req.query.error;
  let errors = {};
  ...
  return res.render('epsv14/search-nhs', {
    'errors': errors
  });
});
  ...
  router.post("/epsv14/search-basic-post", function (req, res) {
...
//get basic route
router.get('/epsv14/search-basic', function (req, res) {
  ...
  return res.render('epsv14/search-basic', { errors });
});


// change record via a NHS number on NoK section
  router.post("/epsv14/change-record", function (req, res) {
  ...
  router.get('/epsv14/patient-details', function (req, res) {
    ...
    return res.render('epsv14/patient-details', {'target': target})
  })

  // switcher routing

  router.post('/epsv14/switch', function (req, res) {
    // grab the bit of the url we need to se the redirect to the same page
    var url = req.get('Referrer') .split( '/' );
    res.redirect('/epsv14/' + url[4] )
  })
 
  
 // Search results page
router.get("/epsv14/search-results", function (req, res, next) {
  ...
  res.render('./epsv14/search-results', {
...
//...................................................................................................................
  ...
  router.get("/epsv14/prescription-results", function (req, res, next) {
    ...
    res.render('./epsv14/prescription-results', {
...
//prescription search results page
router.get("/epsv14/prescription-results-pds", function (req, res, next) {
  // Get the details from the url query parameters
  const nhsNumber = req.query["nhsNumber"] ? req.query["nhsNumber"].trim() : '';
  const prescID = req.query["prescID"] ? req.query["prescID"].trim() : '';
  ...
  // Render the search results page with the filtered prescription lists
  res.render('./epsv14/prescription-results-pds', {
    returnedPrescriptionsListAll: returnedPrescriptionsListAll,
    returnedPrescriptionsListCurrent: returnedPrescriptionsListCurrent,
    returnedPrescriptionsListFuture: returnedPrescriptionsListFuture,
    returnedPrescriptionsListExpired: returnedPrescriptionsListExpired,
...
router.get('/epsv14/roles', (req, res) => {
 ...
 // Check if we should skip the role selection page
  if (
    ...
    return res.redirect(`/epsv14/search?selection=${selection}&cards=${cards}&noAccess=${noAccess}&singleRole=${singleRole}&action=${action}&allRoles=${allRoles}`); // Redirect to search page
  }
 ...
 res.render('./epsv14/roles', { selection, cards, noAccess, roles, selectedRole, singleRole, allRoles });
});

// Locum role
router.get('/epsv14/roles-site-search-locum', (req, res) => {
  ...
  if (!selectedRole) {
    return res.redirect('/epsv14/roles'); // Redirect if no role found
  }
...
console.log("Stored Role ID:", req.session.data.roleId);
  res.render('./epsv14/roles-site-search-locum', { locumsites, leedsLocumsites, oneLocumsites, selectedRole }); // Pass locumsites to the template
});

router.get('/epsv14/roles-confirm-locum', (req, res) => {
  ...
  if (!selectedRole) {
    return res.redirect('/epsv14/roles');
  }
  ...
  res.render('./epsv14/roles-confirm-locum', { selectedRole, selectedSite });
});

//confirmed role
router.get('/epsv14/roles-confirm', (req, res) => {
  let roles = req.session.data.roles; // Retrieve roles from session data
  let selectedRole = roles.find(role => role.id === req.query.roleId); // Find role by ID
...
// Save updated roles back to session
req.session.data.roles = roles;
  res.render('./epsv14/roles-confirm', { selectedRole }); // Pass selected role to template
});


router.get('/epsv14/results-none', (req, res) => {
  const initialSearch = req.query.initialSearch; // Get the value from URL query parameters
  res.render('./epsv14/results-none', { initialSearch }); // Pass it to the template
});

router.get('/epsv14/results-none-new', (req, res) => {
  const initialSearch = req.query.initialSearch; // Get the value from URL query parameters
  res.render('./epsv14/results-none-new', { initialSearch }); // Pass it to the template
});
...
//..........................................................................................................
 ...
 router.get("/epsv14/prescription-template", function (req, res) {
