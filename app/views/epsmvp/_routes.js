module.exports = (router) => {
  ...
  router.get('/epsmvp/search', function (req, res) {
    // check for clear
    let refine = req.query.refine || 'false';
    let lastSearch = req.session.data["last-search"];
    let target = 'epsmvp/search';
    if (lastSearch === 'basic') {
      target = 'epsmvp/search-basic';
    } else if (lastSearch === 'advanced') {
      target = 'epsmvp/search-advanced';
    } else {
      target = 'epsmvp/search';
    }
...
router.post("/epsmvp/search-presc-post", function (req, res) {
  ...
  router.post("/epsmvp/search-nhs-post", function (req, res) {
  ...
  router.post("/epsmvp/search-basic-post", function (req, res) {
...
// change record via a NHS number on NoK section
  router.post("/epsmvp/change-record", function (req, res) {
  ...
  router.post("/epsmvp/switch-layout", function (req, res) {
  ...
  router.get('/epsmvp/patient-details', function (req, res) {
    ...
    return res.render('epsmvp/patient-details', {'target': target})
  })

  // switcher routing

  router.post('/epsmvp/switch', function (req, res) {
    // grab the bit of the url we need to se the redirect to the same page
    var url = req.get('Referrer') .split( '/' );
    res.redirect('/epsmvp/' + url[4] )
  })
 
  
 //search results page
  router.get("/epsmvp/search-results", function (req, res, next) {
    ...
    res.render('./epsmvp/search-results', {
        returnedPatientsList: returnedPatientsList
    });
  });

  //prescription search results page
  router.get("/epsmvp/prescription-results", function (req, res, next) {
    ...
    res.render('./epsmvp/prescription-results', {
