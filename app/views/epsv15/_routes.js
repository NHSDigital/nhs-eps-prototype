module.exports = (router) => {
  ...
  router.get('/epsv15/search', function (req, res) {
    let refine = req.query.refine || 'false';
    let lastSearch = req.session.data["last-search"];
    let target = 'epsv15/search';


    if (lastSearch === 'basic') {
      target = 'epsv15/search-basic';
    } else if (lastSearch === 'advanced') {
      target = 'epsv15/search-advanced';
    }
    ...
    return res.render('epsv15/search', {
      'errors': errors
    });
});
...
router.post("/epsv15/search-presc-post", function (req, res) {
...
router.post("/epsv15/search-nhs-post", function (req, res) {
...
router.get('/epsv15/search-nhs', function (req, res) {
  let errorType = req.query.error;
  let errors = {};
  ...
  return res.render('epsv15/search-nhs', {
    'errors': errors
  });
});
  ...
  router.post("/epsv15/search-basic-post", function (req, res) {
...
//get basic route
router.get('/epsv15/search-basic', function (req, res) {
  ...
  return res.render('epsv15/search-basic', { errors });
});


// change record via a NHS number on NoK section
  router.post("/epsv15/change-record", function (req, res) {
  ...
  router.post("/epsv15/switch-layout", function (req, res) {
  ...
  router.get('/epsv15/patient-details', function (req, res) {
    ...
    return res.render('epsv15/patient-details', {'target': target})
  })

  // switcher routing

  router.post('/epsv15/switch', function (req, res) {
    // grab the bit of the url we need to se the redirect to the same page
    var url = req.get('Referrer') .split( '/' );
    res.redirect('/epsv15/' + url[4] )
  })
 
  
 // Search results page
router.get("/epsv15/search-results", function (req, res, next) {
  ...
  res.render('./epsv15/search-results', {
...
//...................................................................................................................
  ...
  router.get("/epsv15/prescription-results", function (req, res, next) {
    ...
    returnedPrescriptionsListFuture = returnedPrescriptionsListAll.filter(item => {
      ...
      const prescription = item[key];
      // how we filter the list down
      return prescription.prescriptionStatus === 'Future issue date' || prescription.prescriptionStatus === 'Future repeat dispense'|| prescription.prescriptionStatus === 'Future eRD issue' || prescription.prescriptionStatus == 'Next repeat ready to download' || prescription.prescriptionStatus == 'Future prescription cancelled';
    }).map(item => {
      const key = Object.keys(item)[0];
      const prescription = item[key];
      return {
    ...
    res.render('./epsv15/prescription-results', {
...
router.get("/epsv15/prescription-results-pds", function (req, res, next) {
  ...
  returnedPrescriptionsListCurrent = returnedPrescriptionsListAll.filter(item => {
    ...
    // how we filter the list down
    return prescription.prescriptionStatus !== 'Future issue date' && prescription.prescriptionStatus !== 'Future prescription cancelled'&& prescription.prescriptionStatus !== 'Future issue date dispense' && prescription.prescriptionStatus !== 'Future repeat dispense'&& prescription.prescriptionStatus !== 'Future eRD issue' && prescription.prescriptionStatus !== 'Expired' && prescription.prescriptionStatus !== 'Claimed'&& prescription.prescriptionStatus !== 'Next repeat ready to download' && prescription.prescriptionStatus !== 'Future prescription cancelled';;
  }).map(item => {
    const key = Object.keys(item)[0];
    const prescription = item[key];
    return {
  ...
  returnedPrescriptionsListFuture = returnedPrescriptionsListAll.filter(item => {
    ...
    const prescription = item[key];
    // how we filter the list down
    return prescription.prescriptionStatus === 'Future issue date' ||prescription.prescriptionStatus === 'Future prescription cancelled'|| prescription.prescriptionStatus === 'Future issue date dispense'|| prescription.prescriptionStatus === 'Future repeat dispense'|| prescription.prescriptionStatus === 'Future eRD issue'|| prescription.prescriptionStatus == 'Next repeat ready to download' || prescription.prescriptionStatus == 'Future prescription cancelled';
  }).map(item => {
    const key = Object.keys(item)[0];
    const prescription = item[key];
    return {
  ...
  // Render the search results page with the filtered prescription lists
  res.render('./epsv15/prescription-results-pds', {
    returnedPrescriptionsListAll: returnedPrescriptionsListAll,
    returnedPrescriptionsListCurrent: returnedPrescriptionsListCurrent,
    returnedPrescriptionsListFuture: returnedPrescriptionsListFuture,
    returnedPrescriptionsListExpired: returnedPrescriptionsListExpired,
...
router.get('/epsv15/roles', (req, res) => {
 ...
 // Check if we should skip the role selection page
  if (
    ...
    return res.redirect(`/epsv15/search?selection=${selection}&cards=${cards}&noAccess=${noAccess}&singleRole=${singleRole}&action=${action}&allRoles=${allRoles}`); // Redirect to search page
  }
 ...
 res.render('./epsv15/roles', { selection, cards, noAccess, roles, selectedRole, singleRole, allRoles });
});

// Locum role
router.get('/epsv15/roles-site-search-locum', (req, res) => {
  ...
  if (!selectedRole) {
    return res.redirect('/epsv15/roles'); // Redirect if no role found
  }
...
console.log("Stored Role ID:", req.session.data.roleId);
  res.render('./epsv15/roles-site-search-locum', { locumsites, leedsLocumsites, oneLocumsites, selectedRole }); // Pass locumsites to the template
});

router.get('/epsv15/roles-confirm-locum', (req, res) => {
  ...
  if (!selectedRole) {
    return res.redirect('/epsv15/roles');
  }
  ...
  res.render('./epsv15/roles-confirm-locum', { selectedRole, selectedSite });
});

//confirmed role
router.get('/epsv15/roles-confirm', (req, res) => {
  let roles = req.session.data.roles; // Retrieve roles from session data
  let selectedRole = roles.find(role => role.id === req.query.roleId); // Find role by ID
...
// Save updated roles back to session
req.session.data.roles = roles;
  res.render('./epsv15/roles-confirm', { selectedRole }); // Pass selected role to template
});


router.get('/epsv15/results-none', (req, res) => {
  const initialSearch = req.query.initialSearch; // Get the value from URL query parameters
  res.render('./epsv15/results-none', { initialSearch }); // Pass it to the template
});

router.get('/epsv15/results-none-new', (req, res) => {
  const initialSearch = req.query.initialSearch; // Get the value from URL query parameters
  res.render('./epsv15/results-none-new', { initialSearch }); // Pass it to the template
});
...
//..........................................................................................................
 ...
 router.get("/epsv15/prescription-template", function (req, res) {
  ...
  if (!prescID || !prescType) {
    ...
    return res.redirect("/epsv15/search");
  }
...
prescriptionNotDispensedFullReason: prescription.fullPrescriptionNotDispensedReason,
  ...
  res.render("./epsv15/prescription-template", {
    prescriptionID: prescription.prescriptionID,
    prescriptionType: prescription.prescriptionType,
    prescriptionERDsupply: prescription.daysSupply, 
    pendingCancellation: prescription.pendingCancellation,
