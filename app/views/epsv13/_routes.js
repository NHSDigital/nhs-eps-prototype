module.exports = (router) => {
  ...
  router.get('/epsv13/search', function (req, res) {
    let refine = req.query.refine || 'false';
    let lastSearch = req.session.data["last-search"];
    let target = 'epsv13/search';


    if (lastSearch === 'basic') {
      target = 'epsv13/search-basic';
    } else if (lastSearch === 'advanced') {
      target = 'epsv13/search-advanced';
    }
    ...
    return res.render('epsv13/search', {
      'errors': errors
    });
});
...
router.post("/epsv13/search-presc-post", function (req, res) {
...
router.post("/epsv13/search-nhs-post", function (req, res) {
...
router.get('/epsv13/search-nhs', function (req, res) {
  let errorType = req.query.error;
  let errors = {};
  ...
  return res.render('epsv13/search-nhs', {
    'errors': errors
  });
});
  ...
  router.post("/epsv13/search-basic-post", function (req, res) {
...
//get basic route
router.get('/epsv13/search-basic', function (req, res) {
  ...
  return res.render('epsv13/search-basic', { errors });
});


// change record via a NHS number on NoK section
  router.post("/epsv13/change-record", function (req, res) {
  ...
  router.post("/epsv13/switch-layout", function (req, res) {
  ...
  router.get('/epsv13/patient-details', function (req, res) {
    ...
    return res.render('epsv13/patient-details', {'target': target})
  })

  // switcher routing

  router.post('/epsv13/switch', function (req, res) {
    // grab the bit of the url we need to se the redirect to the same page
    var url = req.get('Referrer') .split( '/' );
    res.redirect('/epsv13/' + url[4] )
  })
 
  
 // Search results page
router.get("/epsv13/search-results", function (req, res, next) {
  ...
  res.render('./epsv13/search-results', {
...
//...................................................................................................................
  ...
  router.get("/epsv13/prescription-results", function (req, res, next) {
    ...
    res.render('./epsv13/prescription-results', {
...
//prescription search results page
router.get("/epsv13/prescription-results-pds", function (req, res, next) {
  // Get the details from the url query parameters
  const nhsNumber = req.query["nhsNumber"] ? req.query["nhsNumber"].trim() : '';
  const prescID = req.query["prescID"] ? req.query["prescID"].trim() : '';
  ...
  // Render the search results page with the filtered prescription lists
  res.render('./epsv13/prescription-results-pds', {
    returnedPrescriptionsListAll: returnedPrescriptionsListAll,
    returnedPrescriptionsListCurrent: returnedPrescriptionsListCurrent,
    returnedPrescriptionsListFuture: returnedPrescriptionsListFuture,
    returnedPrescriptionsListExpired: returnedPrescriptionsListExpired,
...
router.get('/epsv13/roles', (req, res) => {
 ...
 // Check if we should skip the role selection page
  if (
    ...
    return res.redirect(`/epsv13/search?selection=${selection}&cards=${cards}&noAccess=${noAccess}&singleRole=${singleRole}&action=${action}&allRoles=${allRoles}`); // Redirect to search page
  }
 ...
 res.render('./epsv13/roles', { selection, cards, noAccess, roles, selectedRole, singleRole, allRoles });
});

// Locum role
router.get('/epsv13/roles-site-search-locum', (req, res) => {
  ...
  if (!selectedRole) {
    return res.redirect('/epsv13/roles'); // Redirect if no role found
  }
...
console.log("Stored Role ID:", req.session.data.roleId);
  res.render('./epsv13/roles-site-search-locum', { locumsites, leedsLocumsites, oneLocumsites, selectedRole }); // Pass locumsites to the template
});

router.get('/epsv13/roles-confirm-locum', (req, res) => {
  ...
  if (!selectedRole) {
    return res.redirect('/epsv13/roles');
  }
  ...
  res.render('./epsv13/roles-confirm-locum', { selectedRole, selectedSite });
});

//confirmed role
router.get('/epsv13/roles-confirm', (req, res) => {
  let roles = req.session.data.roles; // Retrieve roles from session data
  let selectedRole = roles.find(role => role.id === req.query.roleId); // Find role by ID
...
// Save updated roles back to session
req.session.data.roles = roles;
  res.render('./epsv13/roles-confirm', { selectedRole }); // Pass selected role to template
});


router.get('/epsv13/results-none', (req, res) => {
...
//..........................................................................................................
 ...
 router.get("/epsv13/prescription-template", function (req, res) {
  ...
  if (!prescID || !prescType) {
    ...
    return res.redirect("/epsv13/search");
  }
...
prescriptionNotDispensedFullReason: prescription.fullPrescriptionNotDispensedReason,
  ...
  res.render("./epsv13/prescription-template", {
    prescriptionID: prescription.prescriptionID,
    prescriptionType: prescription.prescriptionType,
    prescriptionERDsupply: prescription.daysSupply, 
    pendingCancellation: prescription.pendingCancellation,
